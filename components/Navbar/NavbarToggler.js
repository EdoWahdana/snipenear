...
export default function NavbarToggler({ color, ripple, ...rest }) {
    ...
    return (
      <button
        {...rest}
        className={`cursor-pointer text-xl leading-none py-1 px-1.5 rounded-full border border-solid border-transparent bg-transparent block lg:hidden active:bg-transparent focus:bg-transparent ${bgHoverColors[color]} transition-all duration-300`}
        type="button"
        onMouseUp={(e) => {
